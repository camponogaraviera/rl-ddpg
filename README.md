...
[![SonarQube Cloud](https://sonarcloud.io/images/project_badges/sonarcloud-light.svg)](https://sonarcloud.io/summary/overall?id=camponogaraviera_rl-ddpg)
...
[![Lines of Code](https://sonarcloud.io/api/project_badges/measure?project=camponogaraviera_rl-ddpg&metric=ncloc&token=50819a91c44fdf3711563b72fcb9c92f660a35ea)](https://sonarcloud.io/summary/overall?id=camponogaraviera_rl-ddpg)
[![Quality Gate Status](https://sonarcloud.io/api/project_badges/measure?project=camponogaraviera_rl-ddpg&metric=alert_status&token=ed9482e3c688f1291cced24e784b2cd52a1e8461)](https://sonarcloud.io/summary/overall?id=camponogaraviera_rl-ddpg)

[![Bugs](https://sonarcloud.io/api/project_badges/measure?project=camponogaraviera_rl-ddpg&metric=bugs&token=50819a91c44fdf3711563b72fcb9c92f660a35ea)](https://sonarcloud.io/summary/overall?id=camponogaraviera_rl-ddpg)
[![Code Smells](https://sonarcloud.io/api/project_badges/measure?project=camponogaraviera_rl-ddpg&metric=code_smells&token=50819a91c44fdf3711563b72fcb9c92f660a35ea)](https://sonarcloud.io/summary/overall?id=camponogaraviera_rl-ddpg)
[![Duplicated Lines (%)](https://sonarcloud.io/api/project_badges/measure?project=camponogaraviera_rl-ddpg&metric=duplicated_lines_density&token=50819a91c44fdf3711563b72fcb9c92f660a35ea)](https://sonarcloud.io/summary/overall?id=camponogaraviera_rl-ddpg)

[![Security Rating](https://sonarcloud.io/api/project_badges/measure?project=camponogaraviera_rl-ddpg&metric=security_rating&token=50819a91c44fdf3711563b72fcb9c92f660a35ea)](https://sonarcloud.io/summary/overall?id=camponogaraviera_rl-ddpg)
[![Reliability Rating](https://sonarcloud.io/api/project_badges/measure?project=camponogaraviera_rl-ddpg&metric=reliability_rating&token=50819a91c44fdf3711563b72fcb9c92f660a35ea)](https://sonarcloud.io/summary/overall?id=camponogaraviera_rl-ddpg)
[![Maintainability Rating](https://sonarcloud.io/api/project_badges/measure?project=camponogaraviera_rl-ddpg&metric=sqale_rating&token=50819a91c44fdf3711563b72fcb9c92f660a35ea)](https://sonarcloud.io/summary/overall?id=camponogaraviera_rl-ddpg)
[![Vulnerabilities](https://sonarcloud.io/api/project_badges/measure?project=camponogaraviera_rl-ddpg&metric=vulnerabilities&token=50819a91c44fdf3711563b72fcb9c92f660a35ea)](https://sonarcloud.io/summary/overall?id=camponogaraviera_rl-ddpg)
[![Technical Debt](https://sonarcloud.io/api/project_badges/measure?project=camponogaraviera_rl-ddpg&metric=sqale_index&token=50819a91c44fdf3711563b72fcb9c92f660a35ea)](https://sonarcloud.io/summary/overall?id=camponogaraviera_rl-ddpg)
...
A DDPG-based reinforcement learning agent for finding optimal operating regimes (sweet spots) in [transmon qubits](https://journals.aps.org/pra/abstract/10.1103/PhysRevA.76.042319), maximizing anharmonicity and T2 coherence time.
...
The transmon regime is an operating condition where the ratio between $E_j$ (Josephson energy) and $E_c$ (charging energy) is increased ($E_j/E_c >>1$) to make the solid-state qubit insensitive to charge fluctuations. One way of achieving this regime is by increasing the overall capacitance value $C_{\Sigma}$, thereby decreasing $E_c$. In this regime, the anharmonicity is approximately $\alpha = \omega_{12} - \omega_{01} \approx -E_c$.

However, there is a trade-off. For very large $E_j/E_c$ (small $|\alpha|$), the system becomes nearly harmonic, and leakage to higher energy levels during fast gates increases. For smaller $E_j/E_c$ (larger $|\alpha|$), charge dispersion increases and the qubit moves toward the charge-sensitive (Cooper-pair box) regime.
...
- [technology_stack.md](docs/technology_stack.md).
...
The [TransmonQubitEnv](src/rl_ddpg/envs/transmon.py) **Environment Class** is designed to be Gym-compliant, and it depends on [scQubits](https://github.com/scqubits). It models a transmon qubit with three tunable parameters:
...
- [DDPGAgent](src/rl_ddpg/rl/agent.py) **Class**: Includes methods to select actions, store transitions in a replay buffer, train actor and critic networks, update soft target networks, and save/load model weights.
  - [\_build_networks()](src/rl_ddpg/rl/agent.py#L146) **Method**: Build networks with placeholders to load model weights.
  - [\_scale_action()](src/rl_ddpg/rl/agent.py#L162) **Method**: Linearly scale actions from [-1, 1] to the environment's action space.
  - [\_update_target_networks()](src/rl_ddpg/rl/agent.py#L176) **Method**: Updates Target Actor and Target Critic networks using the `polyak averaging update rule`.
  - [get_action()](src/rl_ddpg/rl/agent.py#L198) **Method**: Get an action from the Policy network and add Gaussian noise for exploration.
  - [store_transition()](src/rl_ddpg/rl/agent.py#L241) **Method**: Uses the [ReplayBuffer](src/rl_ddpg/rl/replay_buffer.py) class to store episode transitions in pre-allocated NumPy arrays within a Python dictionary.
  - [learn()](src/rl_ddpg/rl/agent.py#L265) **Method**: Implemented using `TensorFlow 2 GradientTape API`.
    - Trains the Critic network by minimizing the `Mean-squared Bellman Error (MSBE) loss` with `gradient descent` via Adam optimizer.
    - Trains the Actor network by maximizing the `Deterministic Policy Gradient (DPG) loss` with `gradient ascent` via Adam optimizer.
  - [save_model()](src/rl_ddpg/rl/agent.py#L330) **Method**: Save the full model, including architecture and weights.
  - [load_model_weights()](src/rl_ddpg/rl/agent.py#L372) **Method**: Load model weights only (no optimizer state).
  - [load_full_models()](src/rl_ddpg/rl/agent.py#L404) **Method**: Load full models (architecture + weights + optimizer state).
...
The [ActorNetwork](src/rl_ddpg/rl/networks.py) is a multilayer perceptron (MLP) with two hidden layers implemented using `TensorFlow 2 Model Subclassing API`.
...
The [CriticNetwork](src/rl_ddpg/rl/networks.py) is a multilayer perceptron (MLP) with two hidden layers implemented using `TensorFlow 2 Model Subclassing API`.
...
<details>
  <summary><h1 id="">Install Dependencies</h1></summary>

## CPU Environment

```bash
conda env create -f cpu_environment.yml && conda activate rl-ddpg-cpu
```

## GPU Environment

```bash
conda env create -f gpu_environment.yml && conda activate rl-ddpg-gpu
```
...
Note: Keras is shipped with TensorFlow.
...
<!-- #region Environment Compliance -->
<details>
  <summary><h1 id="env-comp">Environment Compliance</h1></summary>
...
```bash
python -m src.scripts.transmon_regime
```

</details>
<!-- #endregion Environment Compliance -->
...
```bash
python -m pip uninstall rl_ddpg
```
...
<!-- #region CLI -->
<details>
  <summary><h1 id="cli-usage">Command-line interface (CLI) Usage</h1></summary>

- Note: The CLI requires package installation. Alternatively, execute it directly as a Python module:

```bash
python -m rl_ddpg.cli --help
```

```ShellSession
usage: rl_ddpg [-h] [--train] [--render] [--inference]

options:
  -h, --help      Show help message and exit.
  --train         Run training and save energy plots and model weights.
...
- Train the DDPG agent with hyperparameters defined in `configs/config.cfg` (uses a global seed for reproducibility):

```bash
rl_ddpg --train --render
```

- Run inference with pre-trained model weights (no training):
...
<!-- #region Python API Usage -->
<details>
  <summary><h1 id="api-usage">Python API Usage</h1></summary>

Note: Works without package installation.

```python
import rl_ddpg as ddpg
ddpg.about()

# Seed Python, NumPy, and TensorFlow before instantiating the agent, so that the
# network weight initialization is reproducible (defaults to the seed in config.cfg):
ddpg.set_global_seed()

# Instantiate environment and agent with default params from configs/config.cfg:
env = ddpg.TransmonQubitEnv()
agent = ddpg.DDPGAgent()

# Train:
score_history = ddpg.train(env=env, agent=agent, render_flag=True)
```

</details>
<!-- #endregion Python API Usage -->

---

<!-- #region Contribution Guidelines -->
<details>
  <summary><h1 id="lint">Contribution Guidelines</h1></summary>

Lint and format your code before opening a pull request:
