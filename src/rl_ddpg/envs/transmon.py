...
# Defaults will read from configs/config.cfg, so that instantiating the environment
# without arguments matches the values used by the `rl_ddpg --train` command line:
...
class TransmonQubitEnv(gym.Env):
    ...
    def __init__(
        self,
        min_ej: float = _ENV_DEFAULTS["min_ej"],
        max_ej: float = _ENV_DEFAULTS["max_ej"],
        min_ec: float = _ENV_DEFAULTS["min_ec"],
        max_ec: float = _ENV_DEFAULTS["max_ec"],
        min_ng: float = _ENV_DEFAULTS["min_ng"],
        max_ng: float = _ENV_DEFAULTS["max_ng"],
