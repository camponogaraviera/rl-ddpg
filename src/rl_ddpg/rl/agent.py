...
from rl_ddpg.rl.networks import ActorNetwork, CriticNetwork
from rl_ddpg.rl.replay_buffer import ReplayBuffer
from rl_ddpg.utils import agent_params, env_params, min_max_norm_env, net_params

# Type:
CheckpointMode = Literal["full", "weights", "both"]

# Defaults will read from configs/config.cfg, so that instantiating the agent without
# arguments matches the hyperparameters used by the `rl_ddpg --train` command line:
_ENV_DEFAULTS = env_params()
_NET_DEFAULTS = net_params()
_AGENT_DEFAULTS = agent_params()


def _default_obs_space() -> gym.spaces.Box:
    """Build the default observation space, mirroring TransmonQubitEnv."""

    return gym.spaces.Box(
        low=np.array(
            [
                _ENV_DEFAULTS["min_ej"],
                _ENV_DEFAULTS["min_ec"],
                _ENV_DEFAULTS["min_ng"],
            ],
            dtype=np.float32,
        ),
        high=np.array(
            [
                _ENV_DEFAULTS["max_ej"],
                _ENV_DEFAULTS["max_ec"],
                _ENV_DEFAULTS["max_ng"],
            ],
            dtype=np.float32,
        ),
        dtype=np.float32,
...
def _default_action_space() -> gym.spaces.Box:
    """Build the default action space, mirroring TransmonQubitEnv."""
...
class DDPGAgent:
    ...
    def __init__(
        ...
        lr_actor: float = _AGENT_DEFAULTS["lr_actor"],
        lr_critic: float = _AGENT_DEFAULTS["lr_critic"],
        rho: float = _AGENT_DEFAULTS["rho"],
        gamma: float = _AGENT_DEFAULTS["gamma"],
        noise: float = _AGENT_DEFAULTS["noise"],
        buffer_size: int = _AGENT_DEFAULTS["buffer_size"],
        batch_size: int = _AGENT_DEFAULTS["batch_size"],
        ...
        same hyperparameters used by the `rl_ddpg --train` command line.

        Args:
            obs_space: Observation space of the environment.
                Defaults to TransmonQubitEnv's observation space.
            action_space: Action space of the environment.
                Defaults to TransmonQubitEnv's action space.
        ...
        if action_space is None:
            action_space = _default_action_space()
        self.action_space = action_space
        self.num_actions = action_space.shape[0]

        # Hyperparameters:
        self.layer_act_dims = list(layer_act_dims or _NET_DEFAULTS["layer_act_dims"])
        self.layer_crit_dims = list(layer_crit_dims or _NET_DEFAULTS["layer_crit_dims"])
        ...
        self.actor_net = ActorNetwork(
            self.layer_act_dims, num_actuators=self.num_actions, name="actor"
        )
        self.critic_net = CriticNetwork(self.layer_crit_dims, name="critic")
        self.target_actor_net = ActorNetwork(
            self.layer_act_dims, num_actuators=self.num_actions, name="target_actor"
        )
        self.target_critic_net = CriticNetwork(
            self.layer_crit_dims, name="target_critic"
        )
        ...
        self.actor_net.compile(optimizer=Adam(learning_rate=self.lr_actor))
        self.critic_net.compile(optimizer=Adam(learning_rate=self.lr_critic))
